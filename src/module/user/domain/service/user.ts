...
import { UserFilterDTO, CreateUserDTO, UpdateOneUserDTO, UserPublicFilterDTO, UserPublicDTO } from "../dto/user";
...
interface UseUserPublicOptions {
    filter?: UserPublicFilterDTO;
    fetcherOptions?: IFetcherOptions<{ pagination: Pagination; users: UserPublicDTO[] }>;
}
...
export class UserService {
    ...
    static USER_PUBLIC_KEY = "USER_PUBLIC";
    ...
    static useUserPublic = ({ filter = {}, fetcherOptions = {} }: UseUserPublicOptions = {}) => {
        const { data, isPending, isFetching, isError, error } = useFetcher(
            [this.USER_PUBLIC_KEY, filter],
            () => UserApi.getUsersPublic(filter),
            fetcherOptions
        );
